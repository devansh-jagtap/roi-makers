  ...
  try {
    ...
    try {
      ...
      await prisma.lead.update({ 
        where: { id: lead.id }, 
        data: { 
          roiNotificationStatus: result[0].status === 'fulfilled' ? NotificationStatus.SENT : NotificationStatus.FAILED, 
          customerNotificationStatus: result[1].status === 'fulfilled' ? NotificationStatus.SENT : NotificationStatus.FAILED 
        } 
      });
    } catch (emailError) {
      console.error('Lead email process failed', emailError);
      await prisma.lead.update({ 
        where: { id: lead.id }, 
        data: { 
